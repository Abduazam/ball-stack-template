                ...
                @foreach($permissions as $group => $collection)
                    <div class="col-12 pb-4">
                        <div class="d-flex align-items-center">
                            ...
                            <input wire:change="setGroupPermission('{{ $collection->pluck('id')->implode(',') }}')" class="form-check-input" type="checkbox" id="{{ $group }}" name="{{ $group }}" @if($collection->pluck('id')->diff($this->form->permissions)->isEmpty()) checked @endif>
                        </div>
                        <hr class="my-2">
