...
<x-sections.block title="{{ trans('fields.blocks.titles.permissions') }}" export="{{ RoutePathEnum::PERMISSION->value . 'export' }}">
        ...
        <x-forms.tables.body>
            @foreach($permissions as $permission)
                <x-forms.tables.tr wire:key="permission-row-{{ $permission->id }}">
                    <x-forms.tables.td>
                        @can('show', $permission)
                            <x-forms.buttons.link route="{{ route('dashboard.management.permissions.show', $permission) }}" class="btn-secondary">
