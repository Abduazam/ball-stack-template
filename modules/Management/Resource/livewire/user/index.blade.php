...
<x-sections.block title="{{ trans('fields.blocks.titles.users') }}" export="{{ RoutePathEnum::USER->value . 'export' }}">
        ...
        <x-forms.tables.body>
            @foreach($users as $user)
                <x-forms.tables.tr wire:key="user-row-{{ $user->id }}">
                    <x-forms.tables.td>
                        @can('show', $user)
                            <x-forms.buttons.link route="{{ route('dashboard.management.users.show', $user) }}" class="btn-secondary">
                                <x-actions.icons.show/>
                            </x-forms.buttons.link>
                        @endcan

                        @can('update', $user)
                            <x-forms.buttons.link route="{{ route('dashboard.management.users.edit', $user) }}" class="btn-primary">
