...
<x-sections.block title="{{ trans('fields.blocks.titles.roles') }}" export="{{ RoutePathEnum::ROLE->value . 'export' }}">
        ...
        <x-forms.tables.body>
            @foreach($roles as $role)
                <x-forms.tables.tr wire:key="role-row-{{ $role->id }}">
                    <x-forms.tables.td>
                        @can('show', $role)
                            <x-forms.buttons.link route="{{ route('dashboard.management.roles.show', $role) }}" class="btn-secondary">
                                <x-actions.icons.show/>
                            </x-forms.buttons.link>
                        @endcan

                        @can('update', $role)
                            <x-forms.buttons.link route="{{ route('dashboard.management.roles.edit', $role) }}" class="btn-primary">
