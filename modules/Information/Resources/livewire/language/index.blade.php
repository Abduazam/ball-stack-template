...
<x-sections.block title="{{ trans('fields.blocks.titles.languages') }}" export="{{ RoutePathEnum::LANGUAGE->value . 'export' }}">
        ...
        <x-forms.tables.body>
            @foreach($languages as $language)
                <x-forms.tables.tr wire:key="language-row-{{ $language->id }}">
                    <x-forms.tables.td>
                        @can('show', $language)
                            <x-forms.buttons.link route="{{ route('dashboard.information.languages.show', $language) }}" class="btn-secondary">
                                <x-actions.icons.show/>
                            </x-forms.buttons.link>
                        @endcan

                        @can('update', $language)
                            <livewire:information.language.update wire:key="{{ 'update-language-id' . $language->id }}" :$language/>
                        @endcan

                        @can('delete', $language)
                            <livewire:information.language.delete wire:key="{{ 'delete-language-id' . $language->id }}" :$language/>
                        @endcan

                        @can('restore', $language)
                            <livewire:information.language.restore wire:key="{{ 'restore-language-id' . $language->id }}" :$language/>
                        @endcan

                        @can('destroy', $language)
                            <livewire:information.language.destroy wire:key="{{ 'destroy-language-id' . $language->id }}" :$language/>
                        @endcan
                    </x-forms.tables.td>
